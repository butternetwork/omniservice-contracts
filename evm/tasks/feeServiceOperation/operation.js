...
task("setFeeReceiver", "Set message fee service address ")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        if (isTron(hre.network.config.chainId)) {
            ...
            console.log(
                `Update chain ${hre.network.name} change feeRecevier address: ${await feeService.feeReceiver().call()}`,
            );
        } else {
            ...
            if (taskArgs.receiver === "latest") {
                if (currentReceiver == ChainFeeList.feeRecevier) {
                    ...
                    console.log(
                        `Update chain ${hre.network.name} change feeRecevier address: ${await feeService.feeReceiver()}`,
                    );
                }
            } else {
...
task("setMessageFee", "set chain message fee")
        ...
        if (isTron(hre.network.config.chainId)) {
            if (taskArgs.chainid === "latest") {
                for (let chainListFee of Object.entries(ChainFeeList.chainIdList)) {
                    ...
                    console.log(
                        `Update to ${chainListFee[0]} baseGas is: ${await feeService.baseGas(chainListFee[0])}`,
                    );
                }
            } else {
                await feeService
                    .setBaseGas(taskArgs.chainid, ChainFeeList[hre.network.name].chainIdList[taskArgs.chainid].baseGas)
                    .send();
            }
            let gasPrice;
            if (taskArgs.token === "latest") {
                    ...
                    for (let tokenListFee of Object.entries(chainListFee[1].chainGasPrice)) {
                        ...
                        if (gasPrice == tokenListFee[1]) {
                            ...
                            await feeService
                                .setChainGasPrice(
                                    chainListFee[0],
                                    getEvmAddress(hre.network.name, tokenListFee[0]),
                                    tokenListFee[1],
                                )
                                .send();
                            console.log(
                                `Update chain: ${chainListFee[0]}, token: ${tokenListFee[0]}, chainGasPrice is: ${await feeService.chainGasPrice(chainListFee[0], tokenListFee[0]).call()}`,
                            );
                        }
                    }
                }
                ...
                for (let chainListFee of Object.entries(ChainFeeList.chainIdList)) {
                    ...
                    if (gasPrice == chainListFee[1].chainGasPrice[taskArgs.token]) {
                        console.log(
                            `Skip chain: ${chainListFee[0]}, token: ${taskArgs.token}, chainGasPrice is: ${gasPrice}`,
                        );
                    } else {
                        await feeService
                            .setChainGasPrice(
                            ...
                            .send();
                        console.log(
                            `Update chain: ${chainListFee[0]}, token: ${taskArgs.token}, chainGasPrice is: ${await feeService.chainGasPrice(chainListFee[0], taskArgs.token).call()}`,
                        );
                    }
                }
            }
            ...
            if (taskArgs.chainid === "latest") {
                ...
                if (baseGas == ChainFeeList.chainIdList[taskArgs.chainid].baseGas) {
                    ...
                    console.log(
                        `Update to ${taskArgs.chainid} baseGas is: ${await feeService.baseGas(taskArgs.chainid)}`,
                    );
                }
            }
            ...
            if (taskArgs.token === "latest") {
                for (let chainListFee of Object.entries(ChainFeeList.chainIdList)) {
                    for (let tokenListFee of Object.entries(chainListFee[1].chainGasPrice)) {
                        ...
                        if (gasPrice == tokenListFee[1]) {
                            console.log(
                                `Skip chain: ${chainListFee[0]}, token: ${tokenListFee[0]}, chainGasPrice is: ${gasPrice}`,
                            );
                        } else {
                            await (
                                await feeService.setChainGasPrice(chainListFee[0], tokenListFee[0], tokenListFee[1])
                            ).wait();
                            console.log(
                                `Update chain: ${chainListFee[0]}, token: ${tokenListFee[0]}, chainGasPrice is: ${await feeService.chainGasPrice(chainListFee[0], tokenListFee[0])}`,
                            );
                        }
                    }
                }
                ...
                for (let chainListFee of Object.entries(ChainFeeList.chainIdList)) {
                    ...
                    if (gasPrice == chainListFee[1].chainGasPrice[taskArgs.token]) {
                        ...
                        console.log(
                            `Update chain: ${chainListFee[0]}, token: ${taskArgs.token}, chainGasPrice is: ${await feeService.chainGasPrice(chainListFee[0], taskArgs.token)}`,
                        );
                    }
                }
            }
...
task("setToChainGasPrice", "set chain message fee")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        if (isTron(hre.network.config.chainId)) {
            ...
            console.log(
                `Update to chain: ${taskArgs.tochainid}, token: ${taskArgs.token}, chainGasPrice is: ${await feeService.chainGasPrice(taskArgs.tochainid, taskArgs.token).call()}`,
            );
        } else {
            let gasPrice = await feeService.chainGasPrice(taskArgs.tochainid, taskArgs.token);
            if (gasPrice == taskArgs.price) {
                console.log(
                    `Skip to chain: ${taskArgs.tochainid}, token: ${taskArgs.token}, chainGasPrice is: ${gasPrice}`,
                );
            } else {
                await (await feeService.setChainGasPrice(taskArgs.tochainid, taskArgs.token, taskArgs.price)).wait();
                console.log(
                    `Update to chain: ${taskArgs.tochainid}, token: ${taskArgs.token}, chainGasPrice is: ${await feeService.chainGasPrice(taskArgs.tochainid, taskArgs.token)}`,
                );
            }
        }
    });

task("setBaseFee", "set chain message fee")
    .addParam("tochainid", "to chain id", "latest", types.string)
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        if (isTron(hre.network.config.chainId)) {
            ...
            console.log(
                `Update to ${taskArgs.tochainid} baseGas is: ${await feeService.baseGas(taskArgs.tochainid).call()}`,
            );
        } else {
            ...
            if (baseGas == taskArgs.fee) {
                ...
                console.log(
                    `Update to ${taskArgs.tochainid} baseGas is: ${await feeService.baseGas(taskArgs.tochainid)}`,
                );
            }
        }
    });
