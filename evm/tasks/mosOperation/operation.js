    ...
    .setAction(async (taskArgs, hre) => {
        ...
        if(taskArgs.relay == "latest"){
            if (isTron(hre.network.config.chainId)){
                await mos.setRelayContract(taskArgs.chain,await getEvmAddress(hre.network.name,addressList["mosAddress"])).send();
                console.log(`set  relay ${await mos.relayContract().call()} with chain id ${await mos.relayChainId().call()} successfully `);
            }else{
                await (await mos.setRelayContract( taskArgs.chain, addressList["mosAddress"])).wait();
                console.log(`set  relay ${await mos.relayContract()} with chain id ${await mos.relayChainId()} successfully `);
            }
        }else {
            if (isTron(hre.network.config.chainId)){
                await mos.setRelayContract( taskArgs.chain, await getEvmAddress(hre.network.name,taskArgs.relay)).send();
                console.log(`set  relay ${await mos.relayContract().call()} with chain id ${await mos.relayChainId().call()} successfully `);
            }else {
...
task("relayRegisterChain",
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        console.log(`register  chain id ${taskArgs.chain} with mos address ${await mos.mosContracts(taskArgs.chain)} successfully `);
    })