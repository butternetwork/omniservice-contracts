...
//        return lightNode.verifyProofData(_receiptProof);
    ...
    function finalizedState(uint256 _chainId, bytes memory _data) external override view returns (bytes memory){
        bytes memory b = abi.encode("0");
        return b;
    }

    function isVerifiable(uint256 _chainId, uint256 _blockHeight, bytes32 _hash) external override view returns (bool){
        return true;
    }

    function nodeType(uint256 _chainId) external override view returns (uint256){
        return 0;
    }
}